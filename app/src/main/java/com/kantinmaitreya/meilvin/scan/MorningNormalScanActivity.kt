...
class MorningNormalScanActivity:AppCompatActivity() {
    ...
    private fun updateStatus(nis: String?){
        ...
        val dataServer = listOf("nis" to nis,"porsi_pagi" to "0")
                ...
                try {
                    ...
                    when(respondData?.status){
                        1 -> {
                            ...
                            when{
                                json != null -> {
                                    val portion = json.siswa.porsi_pagi
                                    when(portion){
                                        "1" -> {
                                            val nisText = resources.getString(R.string.scan_portion, nis, "pagi", "Jumbo")
                                            nis_number?.text = nisText
                                        }
                                        else -> {
                                            val nisText = resources.getString(R.string.scan_portion, nis, "pagi", "Biasa")
                                            nis_number?.text = nisText
                                        }
                                    }
                                    nis_number?.visibility = View.VISIBLE
