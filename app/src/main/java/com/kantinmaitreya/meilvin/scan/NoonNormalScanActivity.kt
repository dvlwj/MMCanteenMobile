...
class NoonNormalScanActivity:AppCompatActivity() {
    ...
    private fun updateStatus(nis: String?){
        ...
        val dataServer = listOf("nis" to nis,"porsi_siang" to "0")
                ...
                try {
                    ...
                    when(respondData?.status){
                        1 -> {
                            ...
                            when{
                                json != null -> {
                                    val portion = json.siswa.porsi_siang
                                    when(portion){
                                        "1" -> {
                                            val nisText = resources.getString(R.string.scan_portion, nis, "siang", "Jumbo")
                                            nis_number?.text = nisText
                                        }
                                        else -> {
                                            val nisText = resources.getString(R.string.scan_portion, nis, "siang", "Biasa")
                                            nis_number?.text = nisText
                                        }
                                    }
                                    nis_number?.visibility = View.VISIBLE
